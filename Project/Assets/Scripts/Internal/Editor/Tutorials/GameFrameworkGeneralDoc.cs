...
public class GFWKGeneralDoc : TutorialWizard
{
    //required//////////////////////////////////////////////////////
    private const string ImagesFolder = "gfwk2/editor/general/";
    private NetworkImages[] m_ServerImages = new NetworkImages[]
    {
        ...
        new NetworkImages{Name = "https://www.gfworksstudio.com/documentations/gfwk2/assets/images/image_33.png", Image = null, Type = NetworkImages.ImageType.Custom},
        new NetworkImages{Name = "https://www.gfworksstudio.com/documentations/gfwk2/assets/images/image_6.png", Image = null, Type = NetworkImages.ImageType.Custom},
        new NetworkImages{Name = "https://www.gfworksstudio.com/documentations/gfwk2/assets/images/image_27.png", Image = null, Type = NetworkImages.ImageType.Custom},
        new NetworkImages{Name = "https://www.gfworksstudio.com/documentations/gfwk2/assets/images/image_14.png", Image = null, Type = NetworkImages.ImageType.Custom},
        new NetworkImages{Name = "https://www.gfworksstudio.com/documentations/gfwk2/assets/images/image_31.jpg", Image = null, Type = NetworkImages.ImageType.Custom},
        new NetworkImages{Name = "https://www.gfworksstudio.com/documentations/gfwk2/assets/images/image_22.jpg", Image = null, Type = NetworkImages.ImageType.Custom},
        new NetworkImages{Name = "https://www.gfworksstudio.com/documentations/gfwk2/assets/images/image_24.png", Image = null, Type = NetworkImages.ImageType.Custom},
        new NetworkImages{Name = "https://www.gfworksstudio.com/documentations/gfwk2/assets/images/image_19.png", Image = null, Type = NetworkImages.ImageType.Custom},
        new NetworkImages{Name = "https://www.gfworksstudio.com/documentations/gfwk2/assets/images/image_29.png", Image = null, Type = NetworkImages.ImageType.Custom},
        new NetworkImages{Name = "https://img.youtube.com/vi/ysYqI4w1vq4/0.jpg", Image = null, Type = NetworkImages.ImageType.Custom},
        new NetworkImages{Name = "https://www.gfworksstudio.com/documentations/gfwk2/assets/images/image_23.png", Image = null, Type = NetworkImages.ImageType.Custom},
    ...
    private readonly GifData[] AnimatedImages = new GifData[]
    {
        ...
        new GifData{ Path = "gfwk-urpcsps.gif"},
    ...
    new Steps { Name = "如何修改 GFWK", StepsLenght = 3, DrawFunctionName = nameof(HowToEditGFWKDoc),
    ...
    new Steps { Name = "事件", StepsLenght = 0, DrawFunctionName = nameof(GFWKEventsDoc) },
    ...
    new Steps { Name = "更新 GFWK", StepsLenght = 0, DrawFunctionName = nameof(UpdateGFWKDoc) },
    ...
    {
        DrawTitleText("游戏框架");
        DrawText("版本： " + GFWKEditor.AssetData.Version);
        DrawYoutubeCover("GFWK 入门视频", GetServerImage(22), "https://www.youtube.com/watch?v=ysYqI4w1vq4");
        DrawTitleText("热门教程");
        ...
        if (DrawLinkText("将加载界面集成到 GFWK"))
        {
            Application.OpenURL("https://www.gfworksstudio.com/en/integrate-loading-screen-to-gfwk-2-0/");
        }
        if (DrawLinkText("将 DestroyIt 集成到 GFWK"))
        {
            Application.OpenURL("https://www.gfworksstudio.com/en/integrate-destroyit-to-gfwk-2-0/");
        }
    }
    ...
    void TerminologyDoc()
    {
        ...
        Space(10);
        DrawHorizontalColumn("GameData", "一个 <i>ScriptableObject</i>，包含大量 GFWK 前端设置，可轻松调整以满足需求并为游戏换皮。选项从是否显示血液的简单开关，到武器和游戏模式信息一应俱全，位于 GFWK 的 <b>Resources</b> 文件夹中。\n\n更多信息请查看 <b>GameData</b> 章节。");

        DrawHorizontalColumn("Player Prefab", "构成 GFWK 玩家控制器的预制体，包含所需的全部脚本、对象和层级结构，\n默认位于 GFWK 的 <b>Resources</b> 文件夹中。\n\n更多信息请查看 <b>玩家预制体</b> 章节。");
        ...
        DrawHorizontalColumn("Unity 顶部菜单", "指 Unity 编辑器顶部的菜单项，<i><b>File、Edit、Assets、GameObject、Components、GFWK 等…</b></i>");
    }

    void HowToEditGFWKDoc()
    {
        if (subStep == 0)
        {
            DrawText("这个问题听起来像是修改 GFWK 会受到某种限制，但事实并非如此。只要清楚要做什么、知道怎么做，理论上没有任何限制，GFWK 包含游戏的完整源代码和资源，一切都可以按开发者意愿修改。\n \n本指南旨在给出修改 GFWK 的理想方式建议，<b>以便将来合并 GFWK 更新时轻松许多。</b>\n \n如果使用 GFWK 已有一段时间，你会知道它最大的问题之一就是更新合并。每个大版本更新都需导入到新的 Unity 工程，因为把新版本导入到已有旧版本 GFWK 的工程中，会丢失此前对游戏所做的全部修改，而且极可能直接搞坏游戏。因此在旧版本工程中应用新版本的改进和修复，唯一可行的办法就是手动合并，逐个脚本检查、逐个预制体比对，堪称噩梦。\n \n这在一定程度上源于 GFWK 的创建方式。由于代码硬编码和引用关系盘根错节，修改代码的唯一途径就是直接改游戏的默认脚本；\n另外与工具类或扩展类素材不同，游戏模板本就预期用户或开发者去修改核心内容，这也加剧了升级到新版本的难度。\n从 1.9 版起，游戏后端设计开始重做，大部分代码支持继承，开发者无需修改默认脚本即可改代码，只需新建一个脚本 ➔ 继承自基类 ➔ 实现自己的改动。\n \n下一页会详细介绍这一点以及如何使用这类代码设计。");
        }
        else if (subStep == 1)
        {
            DrawSuperText("<?background=#FFF>代码修改</background>\n\n从 GFWK 1.9 起，大部分默认脚本采用代码继承设计，<b>继承允许你或你的程序员基于游戏已有的类创建新类，在保持行为一致的前提下指定新的实现，实现代码复用，并独立扩展原有代码。</b>\n \n<b>以下举例说明这种设计如何工作、有何帮助：</b>\n \n在 GFWK 1.9 之前，若想修改武器拾取系统，必须直接改 <b>bl_GunPickUp.cs</b> <i>（默认脚本）</i>，这会导致该脚本无法再自动更新，否则你的改动就会丢失。\n而在 GFWK 1.9 中做同样的修改，无需直接改默认脚本，而是新建一个脚本并继承其基类（本例为 <b>bl_GunPickUpBase</b>）➔ 把默认脚本的代码复制到新脚本中，然后做想要的修改。由于所有引用都指向基类，无需担心引用问题，只需在原本挂载默认脚本的物体上换挂你的新脚本，并移除或卸载默认脚本即可。\n \n<?background=#FFF>如何继承脚本？</background>\n \n若你是有中高级经验的程序员，可能已熟悉这种编程设计，它不仅是优秀代码的应有之义，也让代码更易维护和扩展，并带来多态结构，从而以更少代价实现新功能或变体。\n \n若你不是程序员或不熟悉代码继承，做法很简单：以武器拾取为例新建一个脚本，假设该脚本用于修改武器拾取逻辑，创建后其结构大致如下：");
    ...
    void GameDataDoc()
    {
        ...
        DrawText("使用 GFWK 时你会注意到，文档、readMe.txt 以及许多注释中都频繁提到 <b>GameData</b>。\n若不清楚 <b>GameData</b> 是什么、如何工作或位于何处，这里简要说明：\n\n<b>GameData</b> 是一个 <i>ScriptableObject</i>，包含大量前端设置，可轻松调整以满足需求并为游戏换皮。选项从是否显示血液的简单开关，到武器和游戏模式信息一应俱全。");
        DrawHyperlinkText("<link=asset:Assets/Resources/GameData.asset>GameData</link> 位于 GFWK 的 <b>Resources</b> 文件夹中：");
        DrawServerImage(3);
        DrawNote("也可通过 <b>GFWK 管理器</b> 窗口快速打开，在 Windows 编辑器中按 <b>Ctrl + M</b>，Mac 上按 <b>Command + M</b>。");
    }
    ...
    void OfflineDoc()
    {
        DrawText("GFWK 支持 Photon <b>离线模式</b>，可以直接运行地图场景进行测试，无需经过进入大厅、创建房间、加载地图场景的流程。\n\n当你修改了玩家预制体或武器并想立刻在运行时验证时，这个功能尤其省事，能节省大量时间、提升开发效率。\n\n开关位置在 <b>GameData</b> -> Offline Mode。\n启用后直接打开地图场景点运行即可。");
        DrawNote("离线模式<b>并非</b>用来借助 GFWK 开发单机游戏，它的目的是方便多人在线玩法的开发调试。");
    }
    ...
    void UniversalRPDoc()
    {
        if (subStep == 0)
        {
            DrawText("GFWK 默认使用旧版内置渲染管线。等 URP <i>通用渲染管线</i> 更加标准化之后，GFWK 会将其作为默认渲染管线。目前若要在 GFWK 中使用 URP 或 HDRP，必须手动转换项目，本文档说明具体做法：\n\n<b><size=20>将 GFWK 项目转换为 URP：</size></b>\n\n*<i>本教程假定你的 GFWK 项目基于内置渲染管线，Unity 版本为 2018.4 或更高</i>*\n\n首先需要移除 Post-Processing 包，操作为 Unity 顶部菜单 <b>GFWK -> Tools -> Delete Post-Processing</b>，等待脚本编译完成。\n\n<i>继续下一步。</i>");
        }
        else if (subStep == 1)
        {
        ...
        {
            DrawText("最后还有一处需要设置。\nURP 和 HDRP 中的相机工作机制与内置渲染管线不同。URP/HDRP 中有一个 Base 相机，若想同时渲染其他相机，必须把它设为 <b>Overlay Camera</b> 并加入 <b>Base Camera</b> 的相机 <b>Stack</b> 列表。\n\nGFWK 的玩家使用两个相机，一个只渲染第一人称武器，另一个渲染其余所有内容。因此需要把渲染第一人称武器的那个相机配置为 Overlay Camera，<b>你使用的每个玩家预制体都要照此处理：</b>\n\n1. 在 <b>Project 窗口</b>中选中玩家预制体 <i>GFWK 默认的玩家预制体位于其 Resources 文件夹中</i>，点击 <b>Open Prefab</b> 按钮。");
            ...
            DrawAnimatedImage(4);
            DrawText("至此即可在 URP 下使用 GFWK。记住最后这一步要在你使用的所有玩家预制体上重复执行。");
        }
    }
    ...
    void HDRPDoc()
    {
        if (subStep == 0)
        {
            DrawText("HDRP 是 Unity 面向高端平台的新渲染管线之一，可以调用前沿的实时 3D 渲染技术，提供高保真画质与不妥协的 GPU 性能。GFWK 默认使用内置渲染管线，你也可以手动把项目转换为 HDRP，本指南说明具体做法。");
            DrawSuperText("<b><size=16>将 GFWK 转换为 HDRP</size></b>\n \n本教程假定你的 GFWK 项目基于内置渲染管线，Unity 版本为 2020.1 或更高。教程中提到的部分选项在新版编辑器中位置或名称可能不同，遇到这种情况可查阅对应 Unity 版本的官方指南：\n<?link=https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@14.0/manual/Upgrading-To-HDRP.html>https://docs.unity3d.com/Packages/com.unity.render-pipelines.high-definition@14.0/manual/Upgrading-To-HDRP.html</link>");
            DrawText("配置渲染管线之前必须先移除 <b>Post Processing</b> 包，因为 HDRP 不支持它，HDRP 自带了后处理系统。自动移除的方式为 Unity 顶部菜单 GFWK ➔ Tools ➔ <b>Delete Post-Processing</b>，等待脚本编译完成后继续下一步。");
        }
        else if (subStep == 1)
        {
    ...
    void KillFeedDoc()
    {
        if (subStep == 0)
        {
            ...
            DownArrow();
            DrawText("• GFWK 提供两种在击杀提示中显示击杀事件的模式。一条击杀事件包含三部分信息：被淘汰的玩家名、完成淘汰的玩家名、以及淘汰所用的武器或原因。\n \n淘汰原因有两种显示方式：\n \n<b>武器名称：</b>");
            DrawServerImage(0);
    ...
    void AfkDoc()
    {
        ...
        DrawText("AFK 是 <i>away from keyboard</i> 的缩写，指玩家长时间未与游戏交互的状态。在多人游戏中，AFK 玩家会带来问题，" +
            "例如 GFWK 采用阵营对抗，AFK 玩家相当于白送对方分数；在另一些场景中，AFK 玩家被用来刷等级。因此不少游戏都会配备一套 " +
            "检测 AFK 玩家并在其持续 AFK 超过一定时间后移出服务器或房间的机制。GFWK 内置了该系统，但 <b>默认关闭</b>。\n \n" +
    ...
    void FPArmsMaterial()
    {
        DrawText("通常所有武器模型共用同一个手部模型，只是为不同阵营使用不同材质与贴图。若要在玩家预制体中逐把武器修改手部贴图会很繁琐，GFWK 已经处理了这件事。\n无需手动逐个修改手臂、袖套、手套等材质，你只需" +
            "创建一个配置资源，列出全部手臂材质以及各阵营对应的不同贴图即可。");
        DownArrow();
        DrawText("首先新建一个手臂材质资源：在 <b>Project 窗口</b>中选中要保存该资源的文件夹，<b>右键</b> -> GFWK -> Player -> <b>Arm Material</b>");
        DrawServerImage(4);
        DownArrow();
        DrawText("然后选中新建的资源，在检视面板中会看到一个列表。需要把手臂模型中 <b>随玩家阵营变化贴图</b> 的材质全部加入该列表。以 GFWK 默认手臂模型为例，它有 3 个材质：袖套、皮肤和手套，但只有袖套和手套的贴图会变化，皮肤保持不变，因此列表中只包含这两个材质。\n多数情况下你只需添加手套材质，即新增一个列表项，指定材质，并按阵营添加不同贴图。\n \n设置完成后，材质会在运行时根据玩家出生所属阵营自动切换贴图。");
        DrawServerImage(5);
    }
    ...
    void DrawTeamsDoc()
    {
        DrawText("GFWK 中有多个使用阵营系统的游戏模式，例如 CTF 夺旗与 TDM 团队死斗。默认阵营名为 Delta 与 Recon，你可以修改这两个名称及其代表色：进入 <b>Game Data</b>，找到 Team 区域：\n");
        DrawServerImage(13);
    }
    ...
    void DrawCoins()
    {
        if (subStep == 0)
        {
            DrawHyperlinkText("GFWK 内置虚拟货币系统，包含两种货币：一种通过游戏获得 <i>随经验值</i>，另一种只能用真实货币购买 <i>可通过 <link=https://www.gfworksstudio.com/en/shop/network/shop/>商店插件</link> 或你自建的内购实现</i>。\n \n无需改动代码即可自定义货币的大量属性，例如 <b>货币名称、颜色、图标和面值</b>。\n \n修改这些属性请进入 <link=asset:Assets/Resources/GameData.asset>GameData</link> ➔ Game Coins ➔ 展开要修改的货币。多数属性名已能自解释，以下说明容易混淆的几个：");
            DrawHorizontalSeparator();
            ...
            DownArrow();
            DrawSuperText("<?title=20>如何使用这些货币？</title>\n \n在核心包中，GFWK 默认不将货币用于任何用途，怎么用由你决定：可用于购买武器、特殊皮肤、干员等游戏内物品，也可实现开箱系统，或任何你能想到的玩法。\n \n若想实现商店系统、让玩家购买武器与货币包，有相应插件：\n \n<?link=https://www.gfworksstudio.com/en/shop/network/shop/>商店系统插件</link>\n \n若要接入支付系统以支持真实货币购买，也有对应插件：\n \n<?link=https://www.gfworksstudio.com/en/shop/addons/unity-iap-for-shop/>Unity IAP 插件</link>\n<?link=https://www.gfworksstudio.com/en/shop/network/paypal-for-shop/>Paypal 插件</link>");
        }
        else if (subStep == 1)
        {
            DrawSuperText("若要对玩家钱包执行 <b>增加</b> 或 <b>扣减</b> 指定数量货币这类基础操作，一行代码即可完成：\n \n<?title=#20>增加货币</title>");
            DrawCodeText("bl_GFWK.Coins.GetCoinData(0).Add(100);");
            DrawSuperText("其中 0 表示该货币在 GameData ➔ GameCoins 列表中的索引。\n默认为 0 是经验货币，1 是金币。\n \n<?title=20>扣减货币</title>");
            DrawCodeText("bl_GFWK.Coins.GetCoinData(0).Deduct(100);");
    ...
    void GameModesDoc()
    {
        if (subStep == 0)
        {
            DrawText("GFWK 提供 3 种游戏模式：<b>团队死斗、夺旗和自由混战</b>，各自逻辑位于独立脚本中，路径为 <i>Assets ➔ GFWK ➔ Scripts ➔ GamePlay ➔ GameModes➔*</i>。\n \n各模式都有一些通用属性可在 GameData ➔ Game Modes ➔ * 的检视面板中调整，展开任一模式即可看到全部可配置项。");
        ...
        {
            DrawHyperlinkText("除 GFWK 内置的 3 种模式外，还有更多热门模式以插件形式提供，可另行购买，例如 <i><b>爆破拆除、占点模式、枪战竞速、淘汰模式和确认击杀</b></i>，均可在 <link=https://www.gfworksstudio.com/en/shop/>官方商店</link> 获取。\n \n假设你想做一个自定义游戏模式，该从哪里入手？\n这个问题不好一概而论，因为每个模式的需求、逻辑和玩法都不同，做法自然各异。因此我无法直接教你如何从零设计模式，但可以告诉你怎么把它接入 GFWK。\n \n第一步是为你的游戏模式创建枚举标识，用于在代码中识别该模式。只需在 <b>GameMode.cs</b> 脚本中按模式名添加一个代称，例如：");
            ...
            DownArrow();
            DrawHyperlinkText("接下来要编写负责把 GFWK 与你的游戏模式对接的脚本。GFWK 通过 <color=#DFFF2AFF>IGameMode</color> 接口规定模式主脚本必须实现哪些函数。因此你需要在模式主脚本中 <link=https://docs.microsoft.com/en-us/dotnet/csharp/fundamentals/types/interfaces>实现该接口</link>，并重写其中要求的函数。为便于上手，可参考默认模式脚本 <i>bl_TeamDeathMatch.cs、bl_FreeForAll.cs 等</i> 的写法。\n \n实现 <b>IGameMode</b> 接口及其函数之后，还有一段必须编写的代码：在脚本 <i>即实现接口的那个</i> 的 Awake() 中调用 Initialize()，并在 Initialize() 中用枚举标识注册该模式，如下所示：\n");
    ...
    void DrawPlayerAnimationDoc()
    {
        ...
        {
            DrawText("GFWK 中第一人称与第三人称动画是不同的，武器动画同样如此。第三人称武器不能使用第一人称武器的动画，它需要 humanoid 动画。");
            DrawNote("第三人称武器动画集即一把武器所需的玩家动画，包括待机、装弹、奔跑和开火。");
            DrawText("默认情况下，GFWK 按武器类型 <i>机枪、手枪、狙击枪、手雷等</i> 区分武器动画，同一类型的所有武器共用一套动画。也就是说若你有多把 <b>狙击枪</b>，它们都会播放同一套狙击枪第三人称动画。\n \n如果你想让 <b>某把特定的第三人称武器使用自定义动画集</b>，就需要在玩家 <b>Animator Controller</b> 中新建一个 <b>子状态机</b> 并建立必要的过渡。下面提供一份操作指引，说明如何通过子状态机为第三人称武器应用自定义动画。");
            DrawHorizontalSeparator();
            DrawTitleText("添加自定义武器动画");
            DrawSuperText("首先需要确定要把子状态机加入到哪个 <b>Animator Controller</b>。GFWK 所有玩家预制体默认使用的动画控制器位于 <i>Assets ➔ Art ➔ Animations ➔ Player ➔ Controllers ➔ Player [Controller]</i> <?link=asset:Assets/Art/Animations/Player/Controllers/Player [Controller].controller>点击此处定位</link>。\n \n如果你没有修改过任何玩家预制体的动画控制器，把上方的默认动画控制器拖入下方字段即可；否则拖入你实际使用的那个。");
            Space(10);
    ...
    {
        DrawHyperlinkText("<b>玩家预制体</b> 指包含玩家控制器所需的全部脚本、对象与结构的 Unity 预制体。\n\nGFWK 的玩家预制体放在名为 <b>Resources</b> 的 Unity 特殊文件夹中 <i>位于项目的 Resources 文件夹内</i>。若要修改与玩家相关的任何内容，例如武器位置、动画、脚本属性等，都必须改动这些预制体。\n\n<b>GFWK 默认使用 2 个玩家预制体</b>，在 <b><link=asset:Assets/Resources/GameData.asset>GameData</link></b> 中指定，即 <b>Player1</b> 与 <b>Player2</b>。Player1 用于阵营 1，Player2 用于阵营 2；若游戏模式不是阵营制，例如自由混战，则使用 Player1。\n\n自 1.8 版本起，你还可以按场景覆盖 Player1 与 Player2，也就是说可以给每张地图的每个阵营指定不同的玩家预制体。做法是在地图场景的任意对象上挂载 <b>bl_OverridePlayerPrefab.cs</b> 脚本 ➔ 在该脚本的检视面板中指定玩家预制体即可。");
        DownArrow();
        DrawHyperlinkText("如果你需要更进阶的玩家选择方案，或想加入更多玩家预制体让玩家在游戏内选择角色，可以看看 <link=https://www.gfworksstudio.com/en/shop/addons/player-selector/>Player Selector</link> 插件。\n");
    }
    ...
    void DrawBullets()
    {
        if (subStep == 0)
        {
            DrawText("GFWK 中的子弹使用对象池。与 GFWK 其他池化对象一样，它们登记在 <b>bl_ObjectPooling</b> 脚本中，该脚本挂在每张地图场景的 <b>GameManager</b> 上。\n在 <b>bl_Gun</b> 中你只需填写池化名称。\n\n\n你可能想为某把武器添加新子弹，例如想要不同的拖尾效果，做法如下：\n\n<size=18>复制子弹预制体：</size>\n\n在 GFWK ➔ Content ➔ Prefabs ➔ Weapon ➔ Projectiles ➔ * 中选中某个已有预制体，按 Ctrl + D 复制，Mac 上为 Command + D。\n\n然后在复制出的预制体上做你想要的修改，之后在 <b>bl_ObjectPooling</b> <i>该脚本挂在房间场景的 GameManager 对象上</i> 中新增一个列表项，把该子弹预制体拖入并设置池化名称：");
            DrawServerImage(17);
    ...
    void DrawKitsSystem()
    {
        DrawText("GFWK 有一套简单但实用的补给包系统，玩家可以在对局中投掷和拾取弹药包或医疗包。默认使用 <b>H</b> 键投掷，投出的是弹药包还是医疗包 <i>取决于玩家兵种</i>。");
        DrawServerImage(7);
        DownArrow();
    ...
    void DrawKillZones()
    {
        DrawText("你的地图可能存在不希望玩家越过的边界。GFWK 为此提供的方案是 <b>击杀区</b>：玩家进入后会弹出带倒计时的警告，若在计时归零前没有离开该区域，游戏会自动将其击杀并送回出生点。\n");
        DrawServerImage(21);
    ...
    {
        DrawText("GFWK 允许玩家在运行时修改部分游戏设置，例如画质与控制设置。作为开发者，你需要为这些设置指定默认值，也就是玩家首次进入游戏时采用的值。");
    ...
    void DrawAddonsDoc()
    {
        DrawText("Game Framework 的核心功能设计得相当完整，但不包含 FPS 游戏常见的一些标准功能，例如小地图、登录系统、等级系统、载具、商店等。GFWK 确实提供其中许多功能，以扩展或插件形式提供，让你按需选择。\n \n这些功能不放进默认配置，主要出于价格与灵活性的考虑。若把所有可能的功能都塞进核心包，价格必然被推高，可能超出很多人的承受范围。提供精简的核心包可以把入门价格控制在低位，让从爱好者到专业开发者的更广泛人群都能负担。\n \n模块化方式也尊重不同项目的独特需求与限制。并非每个游戏都需要复杂的账号系统、载具或连杀奖励。让开发者只挑选并集成自己需要的功能，可以避免为不需要的东西付费或耗费精力。这让 GFWK 不仅更可定制，也更轻量高效。\n\n<b>核心包本身功能完整，插件全部可选。</b>\n \n尽管是可选的，这些插件都经过细致设计，能与核心系统无缝衔接。大多数支持自动集成，无需手动编码或配置。你只需选择需要的功能，系统会平滑接入你的项目。");
    ...
    void EditorMenusDoc()
    {
        DrawText("GFWK 提供了一系列独立的 <b>编辑器窗口</b>，位于 Unity 编辑器顶部菜单的 <b>GFWK</b> 根菜单下：");
        DrawServerImage("img-26.png");
        DrawText("以下简要说明各项用途：\n \n<b><size=18>GFWK ➔ Tutorials</size></b>\n \n- 该子菜单列出了 GFWK 的内置编辑器文档与教程，其中 <b>Tutorials ➔ Documentation</b> 是 GFWK 的主文档。\n \n<b><size=18>GFWK ➔ Tools</size></b>\n \n- 该子菜单提供多种操作与辅助窗口，为 GFWK 提供实用的自动化操作。");
        DrawText("<b><size=18>GFWK ➔ Manager</size></b>\n \n- 打开 GFWK 管理器窗口，可查看 GFWK 的前端设置。该窗口还包含一组实用工具窗口，可管理 GFWK 的一些主要设置，例如玩家默认武器配置、武器信息、关卡等。\n \n<b><size=18>GFWK ➔ GFWK</size></b>\n \n- 打开 GFWK 窗口，也就是首次导入 GFWK 时自动弹出的那个窗口。其中包含当前 GFWK 项目的实用信息，例如所用 GFWK 版本、该版本的更新日志、教程链接与联系方式。");
    }
    ...
    void GameInputDoc()
    {
        if (subStep == 0)
        {
            DrawText("射击游戏 <b><size=8>或者说所有游戏</size></b> 的一个关键要素是操作控制。在动作游戏中这一点尤其特殊，因为玩家往往有自己习惯的按键设置。GFWK 默认采用多数 FPS 游戏通用的标准按键方案。\n \n自 1.9 版本起，GFWK 内置了自定义输入管理器，可在运行时通过设置窗口中的菜单重新绑定按键。你可以在 <i>Input Mapped</i> 中定义默认按键，是否允许玩家在游戏内修改由你决定。\n \n如果你想使用第三方输入管理器，GFWK 同样支持。游戏中使用的全部输入都定义在 bl_GameInput.cs 脚本中，修改这些函数中的代码即可指向你自己的输入系统。\n \n通过函数名即可判断某个输入对应哪个游戏动作 <i>开火、装弹、跳跃等</i>。");
        }
        else if (subStep == 1)
        {
        ...
        {
            DrawText("要在 GFWK 与输入管理器中用手柄，需要额外几步操作。\n \n- 首先，若安装 GFWK 时 <i>在安装窗口里</i> 没有覆盖输入设置，你需要修改 Unity 输入设置以添加所需的控制轴。输入管理器附带了一份已配置好的 <b>Input Settings.asset</b>，点击下方按钮即可应用。");
            ...
            if (!File.Exists("ProjectSettings/InputManager-backup.asset"))
            {
                if (Buttons.FlowButton("配置 Unity 输入管理器"))
                {
                    ...
                    {
                        Debug.LogWarning("未找到 GFWK InputSettings 数据。");
                        return;
                    }
            ...
            {
                GUILayout.Label("GFWK 输入设置已集成完成。");
            }
        ...
        {
            DrawText("新建输入映射的方法：在 <i>Project 视图</i> 中进入目标文件夹 -> 右键 -> Create -> GFWK -> Input -> Input Mapped，随后即可看到新建的资源对象，选中它并配置你的手柄或键盘的全部输入。");
    ...
    void CrosshairDoc()
    {
        DrawText("准星是多数射击游戏的基础功能。GFWK 中可以方便地修改准星形状、颜色和大小，还能为每类武器使用不同准星。");
        DownArrow();
    ...
    void FootStepsDoc()
    {
        DrawText("GFWK 的脚步声由地表标签驱动，预定义标签包括 <i>金属、混凝土、泥土、木头和水面</i>。玩家移动时会根据脚下的地表播放对应脚步声。这些音效可以更换，也可以新增地表标签。脚步声还能按玩家预制体分别设置，从而让不同玩家模型有不同音效。");
        DrawTitleText("Change Sounds");
        DrawHyperlinkText("更换 GFWK 默认脚步声的方式：直接替换默认 <link=asset:Assets/Prefabs/Presents/Audio/FootStepsLibrary.asset>FootStepLibrary</link> 中的音频片段。展开 <b>Groups</b> 列表 ➔ 展开对应标签组 ➔ 替换其中的音频片段。");
        DrawServerImage(29);
        DownArrow();
    ...
    void DrawMobileDoc()
    {
        DrawText("如果你的目标平台是移动端，例如 Android 或 iOS，需要注意：GFWK 虽然能在移动平台运行，但构建前还有一些准备工作。Game Framework 默认为了演示效果，按高端设备的高画质来配置，因此默认并未针对移动端优化，而且核心包也不包含任何移动端操作方案。");

        DrawHyperlinkText("因此首先需要接入移动端操作方案。官方为 Game Framework 专门提供了插件 <link=https://www.gfworksstudio.com/en/shop/addons/gfwk-mobile-control/>Mobile Control</link>，其中包含移动端与触屏设备所需的全部按钮与输入，集成是自动的。当然你也可以改用自己偏好的第三方方案。");

        DrawText("其次需要做一些手动优化，与任何移动端项目一样：先移除后处理效果 <b><size=8><i>可在 Unity 包管理器中移除 Post Processing Stack 包</i></size></b>，把 Standard 着色器换成移动端友好的着色器，降低贴图质量与分辨率，当然你新增的关卡、地图、模型也要适配移动端，等等。<b>无需改动任何代码</b>，GFWK 的代码本身已适配移动端并针对低端平台优化过。\n");
    ...
    void PostProcessingDoc()
    {
        if (subStep == 0)
        {
            DrawText("GFWK 默认使用 Unity 的 <b>Post-Processing Stack v2</b>，这是一组作用于相机、用于提升画面效果的滤镜与图像特效。\n\n图像特效确实能显著改善画面观感，但也会带来性能开销。因此建议只在 PC 或主机这类高端平台使用，<b>不要用于移动平台</b>。若目标平台是移动端，应当删除这个包。\n\n首次在项目中导入 GFWK 时，该系统会自动从 Unity 包管理器 UPM 导入。若要 <b>删除</b>，点击以下右键菜单即可：");
            DrawServerImage(11);
    ...
    void ParticlesDecalsDoc()
    {
        if (subStep == 0)
        {
            DrawHyperlinkText("<b><size=22>粒子</size></b>\n\n粒子效果是让游戏画面出彩的重要一环。GFWK 虽然只在少数事件中使用粒子，但出现频率高，所以光好看还不够，还必须经过优化。GFWK 的默认粒子基本只是占位，虽非必须，但推荐替换成更好的效果。\n \n替换或修改特定粒子只需改动其预制体。GFWK 在以下游戏效果中使用 <b>Particle System</b>：\n \n•  枪口火焰\n•  爆炸\n•  子弹命中\n•  <link=asset:Assets/Prefabs/Level/Particles/Prefabs/Impacts/BloodFast.prefab>血液</link>\n•  火焰\n \n \n除位于各第一人称武器内部的 <b>枪口火焰</b> 粒子外，其余粒子都可在项目 Assets 文件夹的 <i>Assets -> Prefabs -> Level -> Particles->*</i> 中找到。");
            DrawNote("修改粒子预制体时请以默认预制体为参照。除 Particle System 组件外，它们还挂有自定义脚本，这些脚本是游戏正常运行所必需的。");
            Space(50);
            DrawText("<b><size=22>弹痕</size></b>\n \nGFWK 没有使用专门的自定义弹痕系统。因为弹痕仅用于子弹痕迹，且实例化极为频繁，专用系统只会带来不必要的性能开销。因此 GFWK 使用简单的 Quad 网格配合简单的透明着色器作为弹痕。\n \n如前所述，GFWK 唯一的弹痕用途是 <b>子弹痕迹</b>。子弹击中碰撞体时生成，显示哪种痕迹取决于被击中碰撞体的标签。GFWK 默认使用 5 种子弹痕迹，对应不同地表 <i>金属、木头、沙地、混凝土以及一种通用痕迹</i>。更换痕迹贴图的方式是修改子弹命中预制体的材质，这些预制体位于 <i>Assets -> Prefabs -> Level -> Particles -> WeaponEffects -> Prefabs->*</i>。");
        }
        else if (subStep == 1)
        {
    ...
    void UpdateGFWKDoc()
    {
        DrawText("GFWK 大约每 3 到 4 个月发布一次大版本更新，这些更新带来新功能、修复与改进。如果你已在使用 GFWK，多半会希望把改进与修复同步到自己的项目中。遗憾的是，受资源本身的性质限制，直接把新版本包导入到旧版本项目上会覆盖你所有的改动，导致此前的工作与进度全部丢失。\n\n把旧版 GFWK 项目更新到新版本所需的工作量取决于多个因素，例如你改动了多少内容、项目所用版本与新版本的差距、新版本中变更的数量等。\n\n下面针对不同情况给出更新 GFWK 版本的方法：");
        Space(10);
        DrawNote("<b><color=#FF0002FF>重要提示：</color>在尝试以下任何方法或对项目做任何更新操作之前，务必先备份整个项目</b>");
        Space(10);
        DrawTitleText("前端改动");
        DrawText("- 如果你只做了前端改动，例如替换 <i>玩家模型、调整属性、添加地图、添加武器、修改 UI 等</i>，而未改动后端代码，更新就简单些：可以在现有项目上导入新版 GFWK 包，只是需要 <b>取消勾选</b> 下列资源。\n\n首先从资源商店或本地磁盘导入 GFWK 更新包。\n\n在 <b>Unity 包导入窗口</b> 中可以勾选要导入的资源，默认全部选中。为避免覆盖你的改动，只需 <b>取消勾选</b> 以下内容：\n\n•  <b>Resources</b> 文件夹 <i>包含其中全部预制体与 GameData</i>\n•  MainMenu 场景 <i>若你修改过 UI</i>\n•  任何你改动过的预制体。\n\nExampleLevel 场景则要导入，因为它几乎肯定包含变更，可以作为你其他地图场景的参照。\n");
        DrawServerImage(12);
        DownArrow();
        DrawTitleText("后端改动");
        DrawText("如果你对 GFWK 核心脚本做过 <b>代码改动</b>，或修改过 <b>GameManager、GameModes、Lobby 等</b> 主要预制体，更新流程就更复杂。\n\n首先，无法直接在当前项目上导入新版本，因为这会还原你所有的改动、丢掉全部工作。要应用新版本的变更，必须手动合并：查看新版本中脚本与预制体的改动，再与自己的项目逐一比对。\n\n这个过程耗时较长，而且合并失误很容易导致游戏报错。因此我给出一个能减少工作量的方法：\n\n•  首先新建一个干净的 Unity 项目，在其中按常规方式导入新版 GFWK 包 <i>同时导入 Photon PUN</i>。这个项目作为参照，用来查看场景、预制体、属性等前端层面的改动。\n\n•  然后在你的既有项目 <i>即修改过的旧版 GFWK 项目</i> 中 <b>模拟</b> 导入新版 GFWK 包。注意是 <b>模拟</b>，<b>不要真正导入</b>。你只需要让 Unity 包导入窗口弹出来，以便查看有哪些文件发生变化。该窗口会列出包中所有将导入的资源，若某个文件在项目中已存在，还会标明包内文件与项目内文件是否不同，这些有改动的文件右侧会显示一个 Refresh 图标：\n");
        DrawServerImage(26);
        ...
        DrawServerImage(27);
        DrawText("把原始文件 <i>即你项目中的那个</i> 放在左侧，新版 GFWK 的文件 <i>即你之前新建项目中的那个</i> 放在右侧。\n\n差异高亮出来后，只需把这些改动合并进你的项目文件 <i>即旧版 GFWK 的文件</i>。\n\n使用独立程序 <b>Diffmerge</b> 还可以比对整个 GFWK 文件夹、分析所有文件，操作为选择 File -> Open Folder Diff... -> 在第一个框中填入旧版 GFWK 文件夹路径，第二个框中填入新版 GFWK 文件夹路径。\n");
        DownArrow();
        DrawText("上述方法对脚本这类文本文件很高效，但比对预制体与场景的差异需要另想办法，得同时打开两个项目对比。好在 Unity 允许同时打开多个编辑器实例，因此可以把旧版项目与新版 GFWK 项目分别在不同编辑器窗口中打开，逐一检视两个项目中预制体与场景的差异。\n");
    ...
    void LocalNotificationsDoc()
    {
        ...
        DrawCodeText("using GFWK.Runtime.UI;\n            ...\n            void ShowNotificationSample()\n            {\n                new GFWKLocalNotification(\"MY NOTIFICATION TEXT HERE\");\n            }");
    }

    void NetworkStats()
    {
        DrawText("GFWK 网络框架可以向客户端广播应用与大厅的网络统计数据，可用于调试游戏网络，也可以把这些数据展示出来以体现游戏的活跃度。\n \nGFWK 中可以查看两类网络数据：\n \n<b><size=22>网络传输统计</size></b>\n \n用于显示当前网络收发数据包以及本地玩家延迟等基础但实用的信息，显示在屏幕左上角。\n \n该功能默认关闭，可在 GameData -> Show Network Stats 中方便地开启。");
    ...
    void KnownIssuesDoc()
    {
        DrawText("本页列出 GFWK 的已知问题。这里关注的不只是缺陷或错误，也包括我们目前无法修复或规避的问题，以及 <b>影响 GFWK 开发流程的问题</b>。以下回答更偏向个人视角而非官方口径，<b>出自主要开发者 GFWorks</b>。");
        DownArrow();

        DrawText("<b><size=22>抽象层</size></b>\n \n对经验丰富的程序员来说，翻阅 GFWK 源码后很快会注意到的一点是，大多类几乎没有抽象层。这在实现或修改游戏功能时尤其成问题，因为唯一的办法是改动原始脚本，于是又引出另一个问题：你再也无法把 GFWK 升级到新版本，因为升级会覆盖你的改动。\n \n那为什么会这样？为什么不写更整洁、可维护、可扩展、便于继承的代码？\n这个问题需要一些背景，我尽量简短说明：\n \n- GFWK 最初并非作为分发给其他开发者的游戏模板，它原本是一个个人项目，甚至算不上游戏，而是一个学习项目。当时的目标是先跑通功能，代码整洁度往后放。直到 2018 年作为游戏模板发布，核心代码的重构才开始，并且至今仍在进行。每次更新不仅加入新功能，也在用更整洁、支持抽象的代码改善既有功能，让模板更模块化、更便于程序员修改。\n \n当然，这项工作远未结束。我很清楚这一点，但 GFWK 功能太多，要为现有功能提供更好的实现，或者至少在不改变行为的前提下让代码更易推断，工作量很大。而且几乎只有我一个人在写代码，因此重写全部内容需要时间，考虑到我并非全职投入 GFWK，所需时间会更长。");
        Space(10);
        DrawText("<b><size=22>URP、HDRP、TMP、输入系统支持</size></b>\n \nUnity 在不断改进，我们也尽量采用其最新技术。Unity 发布的新特性会在将来取代现有方案，例如新的渲染管线 URP 与 HDRP、Text Mesh Pro、新版输入系统等。\n \n但这些特性大多是在去年年中 <i><size=8><color=#76767694>2020 年</color></size></i> 才脱离预览或测试阶段的。在此之前它们不推荐用于正式项目，因此 GFWK 并未采用。如今它们已可用于生产，正是开始使用的时机，当然这需要一定工作量。由于我当时已在开发 Game Framework Next，就先把这些新特性实现在了它里面 <i><size=8><color=#76767694>指 Game Framework Next</color></size></i>。不过这里讨论的是 Game Framework，这些特性是否也会加入它？我不会承诺全部，但 Text Mesh Pro 与新版输入系统的支持已列入后续更新计划。\n \n那 URP 和 HDRP 呢？\nGame Framework 已经支持它们，只是并非默认，需要手动转换项目。更多信息见本文档的通用渲染管线章节。");
        Space(10);
        DrawText("<b><size=22>所有游戏模式支持机器人</size></b>\n \nGFWK 用户另一个常问的问题是 <b>为什么机器人不是所有游戏模式都支持？</b>\n \n答案可能让你失望：单纯因为工作量太大。\n是的。人工智能本就是棘手的领域，再加上多人联机就更复杂。即便是 GFWK 这类基础机器人，开发也很费功夫。不过 GFWK 核心包确实内置了多人 AI 射击系统，支持两个最常玩的游戏模式。问题在于其余模式都要求机器人按该模式的规则做出定制化行为，这才是复杂且工作量巨大的部分。这让我陷入两难：你可能也能理解，为了持续开发和维护 GFWK，我需要有与投入的工作量和时间相称的收入。而如前所述，AI 开发是我个人觉得最令人沮丧、也最需要时间才能看到进展的方向之一。因此为了收回投入，我只能把它作为插件出售 <i><size=8><color=#76767694>指其他游戏模式的 AI 支持</color></size></i>。\n相比之下，我更愿意把时间放在我认为更合适的其他功能上。\n \n这是否意味着其他游戏模式永远不会支持机器人？\n不是，只是目前还没有排期。");
        Space(10);
    ...
    private static void ShowWindowGFWK()
    {
        EditorWindow.GetWindow(typeof(GFWKGeneralDoc));
    }
}